...
def send_text_message(phoneNumber, mealDict, mealName):
    ...
    message = client.messages.create(
        to='+1' + phoneNumber, 
        from_="+12166665780",
        body="Hey there! It's time to start preparing your %s from Mealy! Here is your recipe:\n\n%s\n\nRating: %s/5\nReady in: %d min\nCalories: %s\nServings: %s\n\nIngredients:\n%s\n\nInstructions:\n%s" % (mealName, mealDict['title'], str(round(mealDict['rating'], 2)), mealDict['minutes'], mealDict['calories'].split(',')[0],  mealDict['calories'].split(',')[1], str(mealDict['ingredients']).replace('[\'','-').replace('[','-').replace('"', '').replace('\']','').replace(']','').replace("\', \'",'\n-').replace(", \'",'\n-'), mealDict['instructions'] ))