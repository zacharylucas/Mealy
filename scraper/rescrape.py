...
oldRecipesPath = 'old-recipes'
oldRecipeFiles = [f for f in listdir(oldRecipesPath) if isfile(join(oldRecipesPath, f))]

# Loop over every ID until we run out
for oldRecipeFile in oldRecipeFiles:
    ...
    print("  File: " + oldRecipeFile)
    id = int(oldRecipeFile.replace('recipe','').replace('.json',''))
    print("  ID: " + str(id))
    ...
    try:
        ...
        fileToRemove = oldRecipesPath + '/' + oldRecipeFile
        remove(fileToRemove)
    except:
        print('  ~~~ERROR~~~')
        continue
